...
public class OfflineStudyTest {
    ...
    @Test public void bundledLessonsWorkOfflineAndPersist() throws Exception {
        try (ActivityScenario<MainActivity> scenario = ActivityScenario.launch(MainActivity.class)) {
            ...
            js(scenario, "window.__check='pending'; fetch('/chinese1.json').then(r=>r.json()).then(async c=>{const a=await fetch(c.vocab.find(w=>w.audio).audio);window.__check=c.lessons.length===10&&c.vocab.length===182&&c.stats.mediaFailed===0&&a.ok&&(await a.arrayBuffer()).byteLength>100?'ok':'bad';}).catch(e=>window.__check=String(e));");
            waitFor(scenario, "window.__check !== 'pending'");
