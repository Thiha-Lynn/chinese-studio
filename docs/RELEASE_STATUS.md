...
- OS build matrix runs in GitHub Actions for macOS, Windows and Linux. Consult the Actions run for its current result.
...
- Google sign-in and account backups are implemented but the live host awaits a Google OAuth client and consent-screen configuration. Account backups use explicit save/load, not automatic multi-device merging.
- The online tutor awaits a replacement server-side API key and selected provider model. It does not run offline.
